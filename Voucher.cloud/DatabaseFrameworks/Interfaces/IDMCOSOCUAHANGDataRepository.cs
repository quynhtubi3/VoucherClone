...
{
    public interface IDMCOSOCUAHANGDataRepository
    {
        List<DMCOSOCUAHANG> Get(ApplicationUser user, JObject model);
        Task<APIResult> GetPagination(ApplicationUser user, JObject model);
        int Delete(ApplicationUser user, DeleteModel model);
        int Save(ApplicationUser user, ref DMCOSOCUAHANG row);
        int Import(ApplicationUser user, bool isAll, List<DMCOSOCUAHANG> rows);
    }
}
