...
(function($){ 	
	...
	h.append("<style type='text/css'>.JCLRgrips{ height:0px; position:relative;} .JCLRgrip{margin-left:-5px; position:absolute; z-index:5; } .JCLRgrip .JColResizer{position:absolute;background-color:red;filter:alpha(opacity=1);opacity:0;width:5px;height:100%;cursor: col-resize;top:0px} .JCLRLastGrip{position:absolute; width:1px; } .JCLRgripDrag{ border-left:1px dotted black;	} .JCLRFlex{} .JCLRgrip.JCLRdisabledGrip .JColResizer{cursor:default; display:none;}</style>");
	...
	var init = function( tb, options){	
		...
		t.BillingualLanguage = ApplicationInstance.Uservar.BillingualLanguage;
	};
	...
	var createGrips = function(t){	
		...
		var th = t.find(">tbody>tr:not('.clusterize-extra-row'):first>th,>tbody>tr:not('.clusterize-extra-row'):first>td"); //table headers are obtained
		...
		th.each(function(i){						//iterate through the table column headers			
			...
			g.t = t; g.i = i; g.c = c; c.w = parseInt($(t.cg[i]).attr("width"));//c.outerWidth();		//some values are stored in the grip's node data as shortcut
	...
	var enableGrips = function (t) {

		t.closest(".clusterize-scroll").find(".JCLRdisabledResize").each(function (i) {
			var dc = t.dc.indexOf(i) != -1;
			if (!dc) {
				//if normal column bind the mousedown event to start dragging, if disabled then apply its css class
				$(this).removeClass('JCLRdisabledGrip').bind('touchstart mousedown', onGripMouseDown);
			} else {
				$(this).addClass('JCLRdisabledGrip');
			}
		});
	}; 
	...
	var syncGrips = function (t){	
		...
		for(var i=0; i<t.ln; i++){	//for each column
			...
			t.g[i].css({			//height and position of the grip is updated according to the table layout
				left: (c.offset().left - t.offset().left + c.outerWidth(false) + (t.cs / 2) - 1) + PX,
	...
	*/
	var syncCols = function(t,i){
		var inc = drag.x - drag.l, c = t.c[i];
		var w = c.w + inc;
		c.width(w + PX);	
		c.w = w;
		t.cg.eq(i).width(w + PX); 
		$(t.cg.eq(i)).attr("width", w);
	...
	*/
    ...
    $.fn.extend({  
		...
		enableGrips: function () {
			let id = $(this).attr(ID);
			let t = tables[id];
			if (t == undefined) return;
			enableGrips(t);
		},				
		translateCol: function (i, i1) {
			let id = $(this).attr(ID);
			let t = tables[id];
			if (t == undefined) return;			
			let c = t.c[i];
			...
			if (c && c1) {
				if (ApplicationInstance.Uservar.BillingualLanguage != 2) {
					if (t.BillingualLanguage == 2 || t.BillingualLanguage == undefined) {
						var w = c.w + c1.w;
						var w1 = 0;	
					} else {
						var w = c1.w;
				...
				} else if (ApplicationInstance.Uservar.BillingualLanguage == 2) {
					var w = (c.w + c1.w) / 2;
					var w1 = w;
				}
				...
				c1.width(w1 + PX);
				c1.w = w1;
				t.cg.eq(i1).width(w1 + PX);
				$(t.cg.eq(i1)).attr("width", w1);	
			}
			t.BillingualLanguage = ApplicationInstance.Uservar.BillingualLanguage;
		},
		syncGrip: function () {
